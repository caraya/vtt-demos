...
function Playr(v_id, v_el) {
    ...
    if (typeof Playr.initialized == "undefined") {
        ...
        Playr.prototype.init = function() {
            ...
            var template = '<div class="playr_captions_wrapper" id="playr_captions_wrapper_' + this.video_id + '">' + '<div class="playr_top_overlay" id="playr_top_overlay_' + this.video_id + '"><a href="http://www.delphiki.com/html5/playr/">Playr</a></div>' + '<div class="playr_video_container" id="playr_video_container_' + this.video_id + '"></div>' + '<div class="playr_captions" id="playr_captions_' + this.video_id + '"></div>' + '</div>' + '<ul class="playr_controls" id="playr_controls_' + this.video_id + '" role="menubar">' + '<li><button class="playr_play_btn" id="playr_play_btn_' + this.video_id + '" tabindex="0"><img src="' + this.base64images.play_control + '" id="playr_play_img_' + this.video_id + '" alt="play" /></button></li>' + '<li>' + '<div class="playr_timebar" id="playr_timebar_' + this.video_id + '" tabindex="0" role="slider" aria-valuemin="0" aria-valuemax="0" aria-valuenow="0">' + '<div class="playr_timebar_buffer" id="playr_timebar_buffer_' + this.video_id + '"></div>' + '<div class="playr_timebar_inner" id="playr_timebar_inner_' + this.video_id + '">' + '<div class="playr_timebar_pos"></div>' + '</div>' + '</div>' + '<span class="playr_timebar_notice" id="playr_timebar_notice_' + this.video_id + '">00:00</span>' + '</li>' + '<li><span id="playr_video_curpos_' + this.video_id + '" role="timer">00:00</span> / <span id="playr_video_duration_' + this.video_id + '">00:00</span></li>' + '<li><button class="playr_mute_btn" id="playr_mute_btn_' + this.video_id + '" tabindex="0"><img src="' + this.base64images.sound_control + '" class="playr_mute_icon" id="playr_mute_icon_' + this.video_id + '" alt="mute" /></button>' + '<div class="playr_volume_ctrl" id="playr_volume_ctrl_' + this.video_id + '" tabindex="0" role="slider" aria-valuemin="0" aria-valuemax="1" aria-valuenow="' + this.config.defaultVolume + '" tabindex="0">' + '<div class="playr_volumebar" id="playr_volumebar_' + this.video_id + '"><div class="playr_volumebar_inner" id="playr_volumebar_inner_' + this.video_id + '"><div class="playr_volumebar_pos"></div></div></div>' + '</div>' + '</li>' + '<li><button class="playr_captions_btn" id="playr_captions_btn_' + this.video_id + '" tabindex="0">Menu</button>' + '<ul class="playr_cc_tracks" id="playr_cc_tracks_' + this.video_id + '">' + '<li class="playr_menu_title">Subtitles & Captions</li>' + '<li class="playr_subtitles_item" id="playr_cc_track_' + this.video_id + '_none">' + '<label for="playr_current_cc_' + this.video_id + '_none">' + '<input type="radio" name="playr_current_cc_' + this.video_id + '" id="playr_current_cc_' + this.video_id + '_none" value="-1" />' + ' None</label>' + '</li>' + '</ul>' + '</li>' + '<li><button class="playr_fullscreen_btn" id="playr_fullscreen_btn_' + this.video_id + '" tabindex="0"><img src="' + this.base64images.fullscreen_control + '" alt="fullscreen" /></button></li>' + '</ul>';
            ...
            for (var i = 0; i < attr.length; i++) {
                // autoplay fix
                if (attr.item(i).nodeName == 'autoplay') this.play();

                // disable native support
                if (attr.item(i).nodeName == 'data-rendering' && attr.item(i).nodeValue == 'playr') this.setNativeTrackSupport(false);
            }
        ...
        Playr.prototype.setNativeTrackSupport = function(nativeSupport) {
            ...
            if (!this.nativeTrackSupport) this.disableNativeTextTracks();
        }
        ...
        Playr.prototype.initEventListeners = function() {
            ...
            document.addEventListener("mozfullscreenchange", function() {
                if (!document.mozFullScreen && that.isTrueFullscreen) {
                    that.fullscreen();
                }
            }, false);
            document.addEventListener("webkitfullscreenchange", function() {
                if (!document.webkitIsFullScreen && that.isTrueFullscreen) {
                    that.fullscreen();
                }
            }, false);

            document.getElementById('playr_play_btn_' + this.video_id).addEventListener('click', function() {
                that.play();
            }, false);

            // timebar events
            document.getElementById('playr_timebar_' + this.video_id).addEventListener('mousedown', function() {
                that.isHoldingTime = true;
            }, false);
            document.getElementById('playr_timebar_' + this.video_id).addEventListener('mouseup', function(e) {
                that.isHoldingTime = false;
                that.setPosition(e, true);
            }, false);
            document.getElementById('playr_timebar_' + this.video_id).parentNode.addEventListener('mousemove', function(e) {
                that.noticeTimecode(e);
                if (that.isHoldingTime) {
                    that.setPosition(e, false);
                };
            }, false);

            // volume control events
            document.getElementById('playr_volumebar_' + this.video_id).addEventListener('mousedown', function() {
                that.isHoldingVolume = true;
            }, false);
            document.getElementById('playr_volumebar_' + this.video_id).addEventListener('mouseup', function(e) {
                that.isHoldingVolume = false;
                that.setVolume(e);
            }, false);
            document.getElementById('playr_volumebar_' + this.video_id).addEventListener('mousemove', function(e) {
                if (that.isHoldingVolume) {
                    that.setVolume(e);
                };
            }, false);
            document.getElementById('playr_mute_btn_' + this.video_id).addEventListener('click', function() {
                that.toggleMute();
            }, false);

            document.getElementById('playr_fullscreen_btn_' + this.video_id).addEventListener('click', function() {
                that.fullscreen();
            }, false);

            // focus handling
            document.getElementById('playr_wrapper_' + this.video_id).addEventListener('focus', function() {
                that.focusedElem = 'playr';
            }, false);
            document.getElementById('playr_play_btn_' + this.video_id).addEventListener('focus', function() {
                that.focusedElem = 'play_btn';
            }, false);
            document.getElementById('playr_timebar_' + this.video_id).addEventListener('focus', function() {
                that.focusedElem = 'timebar';
            }, false);
            ...
            window.addEventListener('resize', function(e) {
                if (that.isFullscreen && !that.isTrueFullscreen) that.updateFullscreen();
            }, false);
        }
            ...
            if (this.nativeTrackSupport) {
                for (var i = 0; i < this.video.textTracks.length; i++) {
                    ...
                    if (t.kind == 'subtitles' || t.kind == 'captions' || t.kind == 'descriptions') {
                        ...
                        var str = '<li class="playr_subtitles_item">' + '<label for="playr_current_cc_' + this.video_id + '_' + i + '">' + '<input type="radio" name="playr_current_cc_' + this.video_id + '" id="playr_current_cc_' + this.video_id + '_' + i + '" value="' + i + '" ' + (t.mode == "showing" ? 'checked="checked"' : '') + ' /> ' + track_label + '</label></li>';
                ...
                if (this.track_tags.length > 0) this.loadTrackContent(0);
            }
        };
                ...
                if (req_track.readyState == 4 && (req_track.status == 200 || req_track.status == 0)) {
                    if (req_track.responseText != '') {
                        ...
                        if (kind == 'subtitles' || kind == 'captions' || kind == 'descriptions') {
                            ...
                            var str = '<li class="playr_subtitles_item"><label for="playr_current_cc_' + that.video_id + '_' + track + '">' + '<input type="radio" name="playr_current_cc_' + that.video_id + '" id="playr_current_cc_' + that.video_id + '_' + track + '" value="' + track + '" /> ' + track_label + '</label></li>';
        ...
        Playr.prototype.setDefaultTrack = function() {
            ...
            if (track_list[to_check]) track_list[to_check].checked = true;
            else track_list[0].checked = true;
            this.setActiveTrack();
        };
        ...
        Playr.prototype.parseTrack = function(track_content, track_kind) {
            ...
            } else if (track_kind == 'chapters') {
                var pattern_identifier = /^chapter-([0-9])+$/;
            }
        ...
        Playr.prototype.displayCaptions = function() {
            ...
            if (this.current_track >= 0) {
                for (i = 0; i < this.subs[this.current_track].length; i++) {
                    if ((this.video.currentTime + this.subtitlesDelay) >= this.subs[this.current_track][i].start && (this.video.currentTime + this.subtitlesDelay) <= this.subs[this.current_track][i].stop) {
                        ...
                        if (prefix) text = '<span class="playr_cue_past">' + text;
                        ...
                        if (this.subs[this.current_track][i].settings != '') {
                            ...
                            if (test_ta) {
                                if (test_ta[2] == 'start') {
                                    captions_styles.push('text-align:left')
                                } else if (test_ta[2] == 'middle') {
                                    captions_styles.push('text-align:center')
                                } else if (test_ta[2] == 'end') {
                                    captions_styles.push('text-align:right')
                                }
                            }
                            ...
                            if (test_vt) {
                                ...
                                text = '<span style="' + captions_letters_styles.join(';') + '">' + text.split('').join('</span><span style="' + captions_letters_styles.join(';') + '">') + '</span>';
                                text = text.replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1</span><br /><span>$2');
